...
class CNN_3D(nn.Module):
    def __init__(self, input_dim, embed_dim, hidden_dim, num_layers, output_dim, batch_size, device, init_params=True):
        ...
        self.module_3d = nn.Sequential(
            ...
            nn.Conv3d(16, 8, kernel_size=(3, 3, 3), stride=(2, 1, 1), padding=(1, 1, 1), bias=False),
            nn.BatchNorm3d(8),
            nn.ReLU(inplace=True),
    ...
    def forward(self, x):
        ...
        self.clips_nums = self.max_seq_len - 8 + 1
        ...
        for b in range(self.batch_size):
            ...
            for i in range(self.clips_nums): # time_step = 200, 200 - 8 + 1 = 193(0-192, 7-199), 193*4 = 772
                split_input.append(output[b:b+1, i:i+8, :, :])
        ...
        cnn_3d_input = cnn_3d_input.contiguous().view(cnn_3d_input.shape[0] * cnn_3d_input.shape[1], 8, 15, 15)
        cnn_3d_input = cnn_3d_input.unsqueeze(1) #in_channel = 1
        #########

        output = self.module_3d(cnn_3d_input) #[batch_size * (max_seq_len - 8), 1, 1, 15, 15]
        ...
        output = prefix_sum.contiguous().view(self.batch_size * (self.max_seq_len - 8 + 1), -1)

        output = self.fc(output) #[batch_size * clips_nums, 110]
